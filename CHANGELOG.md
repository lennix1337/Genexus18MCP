...
- **`genexus_whoami` no longer echoes the static version-catalog entry detail on every health check.** The per-major entry objects (`displayName`, `driver`, `defaultInstallPath`, `registryNames`, `legacyRegistryVersions`) and their legacy counterparts are derived from `config/gx-versions.json` and cannot change during a session, so repeating them cost roughly a third of every whoami response. The identity fields the whoami contract exposes (`source`, `primaryMajor`, `supportedMajors`, `legacyMajors`) are unchanged, and the entry detail is still returned by `genexus_whoami(verbose=true)` — the lean payload points at that channel through the small `geneXus.catalog.entryDetail` hint, and the `verbose` schema description now names the catalog detail it returns. Measured on a real KB (GeneXus 18, `C:\kbs\KBTeste`): **6063 → 3859 wire bytes per call (-36.4%, ~550 tokens)**, median over repeated runs. `genexus_whoami` advertises no `outputSchema`, so no strict client is affected; `GeneXusVersionCatalog.ToDiagnosticObject()` keeps the entry detail by default for the KB-provisioning caller that resolves a declared major to its install path.
